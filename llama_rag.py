...
def generate_report(form_answers: List[str]) -> str:
    ...
    # Progress tracking file for streaming UI
    progress_file = os.path.join(llm_analyses_dir, 'generation_progress.json')

    def update_progress(total, completed, current, questions_content, is_complete=False):
        ...
        progress_data = {
            ...
            "report_path": output_filepath if is_complete else None
        }
    ...
    try:
        ...
        # Initialize progress
        questions_content = []
        update_progress(len(filled_questions), 0, 0, questions_content)
        ...
        with open(output_filepath, 'w', encoding='utf-8') as file:
            for filled_idx, (idx, answer) in enumerate(filled_questions):
                ...
                # Update progress to show current question being processed
                update_progress(len(filled_questions), filled_idx, idx+1, questions_content)
                ...
                # Enhanced prompt that instructs LLM to cite sources but NOT generate its own reference list
...
2. Reference the context documents when applicable using inline citations like [1], [2]
3. Highlight any concerns or strengths in the protocol
4. Be specific and actionable in your analysis
5. IMPORTANT: Do NOT include a separate "References" or "Sources" section at the end - just use inline citations like [1], [2] within your analysis text. The references will be added automatically.
...
Analysis:"""
                ...
                # Post-process LLM output to remove any References/Sources section it generated
                # The LLM sometimes ignores the instruction and adds its own reference list
                def strip_llm_references(text):
                    """Remove any References/Sources sections generated by the LLM"""
                    # Pattern to match common reference section headers and everything after
                    patterns = [
                        r'\n\s*(?:References?|Sources?|Citations?|Bibliography)\s*:?\s*\n.*',  # "References:" followed by content
                        r'\n\s*(?:References?|Sources?|Citations?|Bibliography)\s*:?\s*$',     # "References:" at end
                        r'\n\s*\[\d+\]\s+(?:45 CFR|Belmont|Common Rule|OHRP|IRB|Federal|Code of Federal).*(?:\n\s*\[\d+\].*)*$',  # Numbered reference lists at end
                    ]
                    cleaned = text
                ...
                llm_response_text = strip_llm_references(llm_response_text)
                ...
                # Flush the file so content is written immediately
                file.flush()

                # Add this question's content to progress for streaming UI
                questions_content.append({
                    "question_number": idx + 1,
                    "user_input": answer.strip()[:500] + ('...' if len(answer.strip()) > 500 else ''),  # Truncate for UI
                    "analysis": formatted_analysis,
                    "sources": sources_used
                })

                # Update progress after completing each question
                update_progress(len(filled_questions), filled_idx + 1, idx + 1, questions_content)
                logger.info(f'Completed question {idx+1} ({filled_idx+1}/{len(filled_questions)})')

        logger.info(f'Wrote analysis to {output_filepath}')

        # Mark as complete in progress file
        update_progress(len(filled_questions), len(filled_questions), len(filled_questions), questions_content, is_complete=True)
