...
# container for latest answers (module-level so other modules can read)
form_answers = []

# configure a simple logger that writes to llm_analyses/report_generation.log
log_dir = "llm_analyses"
import os
os.makedirs(log_dir, exist_ok=True)
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s %(levelname)s %(message)s',
    handlers=[logging.FileHandler(os.path.join(log_dir, "report_generation.log")), logging.StreamHandler()]
)


def _start_background_report_generation(answers):
    """Start the LLM/RAG report generation in a background thread.

    This imports the heavy `llama_rag` module lazily and runs generate_report.
    Running in background prevents the Flask request from timing out while the UI shows
    the "Please wait" message.
    """
    def _target():
        try:
            logging.info("Background report generation started")
            # import here to avoid heavy imports on module load
            from llama_rag import generate_report
            output_path = generate_report(answers)
            logging.info(f"Report generation finished: {output_path}")
...
def get_form_inputs(input_list):
    """Store the submitted form inputs and kick off background report generation.

    Returns immediately after starting the background thread.
    """
    global form_answers
    form_answers = input_list
    logging.info("Received form responses; launching background generation")
    # start report generation asynchronously
    _start_background_report_generation(form_answers)
    # return immediately
    return True
